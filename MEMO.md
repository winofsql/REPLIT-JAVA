...
## ✅ 11月24日
  - ### Google Chrome のデベロッパーツール
    - ### ネットワーク
      - localhost の掲示板でテスト
      - GET または POST で送ったデータの内容を見る事が可能
      - そのページから参照されるコンテンツを確認できる
        - 画僧の場合は、新しいタブで表示または、プレビューから表示・コピー・ダウンロード可能( SVG は 画像としてコピーできない )
    - ### アプリケーション
      - localStorage に JavaScript で大きなデータを自由にそのページに対して保存して、ここで確認できる
        - 修正や削除も可能
      - localStorage のデータを使って、ブックマークレットから参照してそのページのテスト用のデータ入力に利用しやすい
      - そのページに保存されていてるクッキーの編集や削除がせ可能
    - ### 設定
      - 言語設定と、デバッガの JavaScript の無効を知っておく
    - ### スマホの機種別に表示をシミュレート      
